...
- [ ] ERベースの地図/位置推定
  - ERに画像＋行動履歴（回転/前進量、ジャイロ値があればそれも）を渡し、推定マップ/ターゲット位置（JSON）を返させるプロンプトを設計。
  - 内部に推定マップ/最後に見たターゲット位置を保持し、ERプランへのコンテキストとして渡す。
- [ ] ERベースの距離/方向推定
  - 連続フレームやBBox高さの履歴、行動ログを入力し、前進時間・角度をERに推定させて`move_forward`/`move_forward_long`を計画させる。
  - ジャイロやオドメトリがあれば併用し、推定精度を上げる。
