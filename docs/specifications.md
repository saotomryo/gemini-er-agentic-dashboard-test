...
*   **汎用プランニング**: 「赤いポールのところへ行って」といった抽象的な指示を、具体的な行動シーケンス（旋回、移動、探索など）に分解して実行します。現行実装は「1アクションずつ計画→実行→再計画」のループで動作し、必要に応じて ER にフォールバックできる設計を志向します。
...
*   **入力**: 自然言語指示、現在の Robot View 画像、必要に応じて直近の周囲4方向画像、行動履歴。
...
*   **高度化（ER連携）**:
    *   Flashプランナーで決定できない場合に ER へ「planner-assist」プロンプトを送り、単一アクション JSON を返すフェイルセーフを追加。
    *   将来的にツールコール（scan/turn/move/approach をツール定義）での複数ステップ提案を許容する設計とする。
*   **メモ**: スキャン実行中は追加のプランニングを行わず、4ビュー取得完了後に再プランする。

### 4.3. Vision System (視覚認識)
*   **モデル**: `gemini-robotics-er-1.5-preview` (または最新のRobotics向けモデル)。
*   **出力フォーマット**:
    *   2Dバウンディングボックスは `[ymin, xmin, ymax, xmax]`、座標は 0–1000 正規化。
    *   未検出時は `[]`（空配列）を返却するプロンプトにする。
*   **機能**:
    *   指定された `target` を画像内から検出し、JSON配列で返す。
    *   進捗推定（オプション）: 直近フレームと履歴から `{"status": IN_PROGRESS|STALLED|COMPLETED, "progress": 0..1, "comment": "..."}`
...
*   **Closed-Loop制御 (Visual Servoing)**:
    ...
    *   将来的にカメラ内点の逆投影による粗ワールド位置推定を使い、目標角度・前進距離を計算する補助ロジックを追加予定（MuJoCoカメラ行列利用）。
...
## 8. 開発計画（ER高度利用の追加タスク）
1. プロンプト整備と正規化: Vision/Planner/進捗推定のプロンプトを正規化（0–1000、空配列）、未検出時は `[]` を返すよう統一。
2. ERプランナー補助: Flashが決定できないケースで ER へフォールバックする「planner-assist」呼び出しを実装（単一アクションJSON返却）。
3. 進捗・スタック検知: ERによる `status/progress/comment` 推定を組み込み、低進捗が続く場合に自動再スキャン/再プランを行う。
4. ワールド粗推定: 正規化点/ボックスを MuJoCo カメラ行列で逆投影し、推定方位と距離から次アクションを補助するモジュールを試作。
5. ツールコール準備: scan/turn/move/approach をツール定義し、将来の ER ツールコールプランニングに備えたインターフェースを設計。
6. パラメータ調整: キャリブレーション結果（離散ターン速度/時間）を反映し、シミュレーションで検証・チューニング。
